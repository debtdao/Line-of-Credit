...
contract SpigotedLine is ISpigotedLine, LineOfCredit, ReentrancyGuard {
    ...
    // 0x exchange venue to trade revenue tokens from a Spigot for credit tokens
    address payable public immutable swapTarget;

    // % of revenue tokens to take from Spigot if the Line of Credit  is healthy
    uint8 public immutable defaultRevenueSplit;

    // credit tokens bought from revenue tokens but not yet withdrawn by Lenders /used to repay debt
   ...
   * @param claimToken - The revenue token escrowed by a Spigot to claim and then trade via a Dex for a credit token to use to repay credit
   * @param zeroExTradeData - data generated by the Dex (0x) API to trade `claimToken` against their exchange contract
  */
