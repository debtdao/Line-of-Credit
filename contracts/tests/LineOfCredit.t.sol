...
contract LineTest is Test, Events {
    ...
    function setupQueueTest(uint256 amount) internal returns (address[] memory) {
    ...
    function test_can_borrow_within_credit_limit(uint256 amount) public {
        ...
        assertEq(
            supportedToken1.balanceOf(lender), mintAmount - amount, "Contract should have initial mint balance minus 1e18"
        );
        bytes32 id = line.ids(0);
    ...
    function test_return_lender_funds_on_deposit_and_close() public {
        ...
        assertEq(
            supportedToken1.balanceOf(lender), mintAmount - 1 ether, "Lender should have initial balance less lent amount"
        );
    ...
    function test_deposit_and_repay_less_than_debt_becomes_liquidatable(uint256 credit) public {
        vm.assume(credit >= 1 ether && credit <= mintAmount);
        _addCredit(address(supportedToken1), credit);
        bytes32 id = line.ids(0);
        hoax(borrower);
